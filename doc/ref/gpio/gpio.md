...
The `chip` instance must be kept alive for as long as any line that references it is alive.

`line` is a generic representation of a GPIO line.
It is recommended to use one of the specializations:
- `input_line`
- `output_line`

`ultrasonic` is measures distance to an obstacle by sending an ultrasound pulse using and `output_line` (trigger) and an `input_line` (echo).

`pwm_emulator` is a software implementation of PWM over a regular output line.
It consumes cycles on the main CPU. 
That is why it should only be used when hardware PWM is not physically available.
