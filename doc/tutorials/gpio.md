...
It is strongly recommended to pass in an `abc::diag::log_ostream` to all `abc::gpio::*` and `abc::smbus::*` instances.
...
## Set/Get the Voltage Level on a Line
Before you can use a line, you need to look up the chip's/device's documentation to find the logical number/position of the line as well as its physical pin where the peripheral should be connected.

Once the peripheral is connected to the correct pin, construct an `abc::gpio::output_line` (assuming the line is an output one):
```c++
abc::gpio::line_pos_t line_pos = 5;
abc::gpio::output_line line(&chip, line_pos, &log);
``` 

### Set the Voltage Level on an `abc::gpio::output_line`
...
### Read the Voltage Level on an `abc::gpio::input_line`
...
## Set the Duty Cycle on a Motor over SMBus (I2C)
### Access an SMBus Controller
Similar to GPIO chips, SMBus controllers are also accessible as system devices - `/dev/i2c-*` - starting with `1`:
...
// Variation 2:
abc::smbus::controller controller("/dev/i2c-1", &log);
```

### Create an SMBus Target
```c++
abc::smbus::address_t addr = 0x14;
abc::smbus::clock_frequency_t clock = 72 * std::mega::num; // 72 MHz
bool swap_bytes = true;

abc::smbus::target hat(addr, clock, swap_bytes);
```
...
### Create a PWM
```c++
abc::smbus::pwm_pulse_frequency_t frequency = 50; // 50 Hz
abc::smbus::register_t reg_motor = 0x0d;
abc::smbus::register_t reg_timer = reg_motor / 4; // This HAT has 4 PWM channels per timer

abc::smbus::register_t reg_pwm_base = 0x20;
abc::smbus::register_t reg_pwm = reg_pwm_base + reg_motor;

abc::smbus::register_t reg_autoreload_base = 0x44;
abc::smbus::register_t reg_autoreload = reg_autoreload_base + reg_timer;

abc::smbus::register_t reg_prescaler_base = 0x40;
abc::smbus::register_t reg_prescaler = reg_prescaler_base + reg_timer;

abc::smbus::pwm pwm(&smbus, smbus_target, frequency, reg_pwm, reg_autoreload, reg_prescaler, &log);
```

### Set a Duty Cycle
```c++
abc::smbus::pwm_duty_cycle_t duty_cycle = 75; // 75%
