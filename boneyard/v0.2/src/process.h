...
namespace abc {
	...
	/*static*/ inline thread_id_t thread::current_thread_id() noexcept {
	...
	/*static*/ inline process_id_t process::current_process_id() noexcept {
		return _current->id();
	}


	/*static*/ inline process& process::current() noexcept {
		return *_current;
	}


	inline process::process(abc::pool<process_id_t>& peer_pool, thread_id_t thread_pool_capacity, process_id_t child_pool_capacity)
	...
	inline abc::pool<thread_id_t>& process::thread_pool() noexcept {
		return _thread_pool;
	}


	inline abc::pool<process_id_t>& process::child_pool() noexcept {
		return _child_pool;
	}
	...
	inline process_kind_t	launcher::kind() const noexcept {
	...
	inline const daemon_container& program::daemons() const noexcept {
		return _daemons;
	}
	...
	inline process_kind_t	program::kind() const noexcept {
	...
	inline daemon::daemon(program& parent, daemon_start_handler&& start_handler, std::size_t heap_size, std::size_t output_size)
	...
	inline void daemon::start(process_cycle_t cycle) {
		_current = this;
	...
	inline process_kind_t	daemon::kind() const noexcept {
