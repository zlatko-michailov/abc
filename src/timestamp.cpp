...
namespace abc {
	...
	basic_timestamp::basic_timestamp() noexcept {
		reset_date(min_year, min_month, min_day);
		reset_time(min_hour, min_minute, min_second, min_nanosecond);
	}
	...
	bool basic_timestamp::reset_date_if_done(date_count_t& year, date_count_t& month, date_count_t& day, date_count_t& remaining_days, date_count_t days_in_1_month) noexcept {
	...
	void basic_timestamp::reset_date(year_t year, month_t month, day_t day) noexcept {
		_year = year;
		_month = month;
		_day = day;
	}
	...
	void basic_timestamp::reset_time(hour_t hours, minute_t minutes, second_t seconds, nanosecond_t nanoseconds) noexcept {
		_hours = hours;
		_minutes = minutes;
		_seconds = seconds;
